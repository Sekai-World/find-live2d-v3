...
export namespace Live2DCubismFramework {
  ...
  export class CubismPhysicsParameter {
    public id: CubismIdHandle; // パラメータ
    public targetType: CubismPhysicsTargetType; // 適用先の種類
  }
  ...
  export class CubismPhysicsNormalization {
    public minimum: number; // 最大値
    public maximum: number; // 最小値
    public defalut: number; // デフォルト値
  }
  ...
  export class CubismPhysicsParticle {
    ...
    public mobility: number; // 動きやすさ
    public delay: number; // 遅れ
    public acceleration: number; // 加速度
    public radius: number; // 距離
  ...
  export class CubismPhysicsSubRig {
    public inputCount: number; // 入力の個数
    public outputCount: number; // 出力の個数
    public particleCount: number; // 物理点の個数
    public baseInputIndex: number; // 入力の最初のインデックス
    public baseOutputIndex: number; // 出力の最初のインデックス
    public baseParticleIndex: number; // 物理点の最初のインデックス
   ...
   * @return スケール値
   */
  export type physicsScaleGetter = (translationScale: CubismVector2, angleScale: number) => number;
  ...
  export class CubismPhysicsInput {
    ...
    public sourceParameterIndex: number; // 入力元のパラメータのインデックス
    public weight: number; // 重み
    public type: number; // 入力の種類
    public reflect: boolean; // 値が反転されているかどうか
    public getNormalizedParameterValue: normalizedPhysicsParameterValueGetter; // 正規化されたパラメータ値の取得関数
    constructor() {
  ...
  export class CubismPhysicsOutput {
    ...
    public destinationParameterIndex: number; // 出力先のパラメータのインデックス
    public vertexIndex: number; // 振り子のインデックス
    public translationScale: CubismVector2; // 移動値のスケール
    public angleScale: number; // 角度のスケール
    public weight: number; // 重み
    public type: CubismPhysicsSource; // 出力の種類
    public reflect: boolean; // 値が反転されているかどうか
    public valueBelowMinimum: number; // 最小値を下回った時の値
    public valueExceededMaximum: number; // 最大値をこえた時の値
    public getValue: physicsValueGetter; // 物理演算の値の取得関数
    public getScale: physicsScaleGetter; // 物理演算のスケール値の取得関数
    constructor() {
  ...
  export class CubismPhysicsRig {
    ...
    public subRigCount: number; // 物理演算の物理点の個数
