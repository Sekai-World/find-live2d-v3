...
export namespace Live2DCubismFramework {
    ...
    export class CubismPhysics {
        ...
        public _physicsRig: CubismPhysicsRig;      // 物理演算のデータ
        public _options: Options;    // オプション
        ...
        public evaluate(model: CubismModel, deltaTimeSeconds: number): void {
            ...
            for (let settingIndex: number = 0; settingIndex < this._physicsRig.subRigCount; ++settingIndex) {
                ...
                for (let i: number = 0; i < currentSetting.outputCount; ++i) {
                    ...
                    translation.x = (currentParticles[particleIndex].position.x as any) - (currentParticles[particleIndex - 1].position.x as any);
                    translation.y = (currentParticles[particleIndex].position.y as any) - (currentParticles[particleIndex - 1].position.y as any);
        ...
        public initialize(): void {
            ...
            for (let settingIndex: number = 0; settingIndex < this._physicsRig.subRigCount; ++settingIndex) {
                ...
                (strand[0].lastGravity.y as any) *= -1.0;
                ...
                for (let i: number = 1; i < currentSetting.particleCount; ++i) {
                    ...
                    strand[i].initialPosition = new CubismVector2((strand[i - 1].initialPosition.x as any) + radius.x, (strand[i - 1].initialPosition.y as any) + radius.y);
                    strand[i].position = new CubismVector2(strand[i].initialPosition.x, strand[i].initialPosition.y);
                    strand[i].lastPosition = new CubismVector2(strand[i].initialPosition.x, strand[i].initialPosition.y);
                    strand[i].lastGravity = new CubismVector2(0.0, -1.0);
                    (strand[i].lastGravity.y as any) *= -1.0;
    ...
    export class Options {
        ...
        public gravity: CubismVector2; // 重力方向
        public wind: CubismVector2;    // 風の方向
        constructor() {
    ...
    function getInputTranslationXFromNormalizedParameterValue(
        ...
        isInverted: boolean,
        weight: number): void {
      (targetTranslation.x as any) += normalizeParameterValue(
            value,
    ...
    function getInputTranslationYFromNormalizedParamterValue(
        ...
        isInverted: boolean,
        weight: number): void {
      (targetTranslation.y as any) += normalizeParameterValue(
            value,
            parameterMinimumValue,
            parameterMaximumValue,
            parameterDefaultValue,
    ...
    function getOutputTranslationX(
      ...
      let outputValue: number = translation.x as any;
    ...
    function getOutputTranslationY(
        ...
        isInverted: boolean,
        parentGravity: CubismVector2): number {
      let outputValue: number = translation.y as any;
    ...
    function updateParticles(
        ...
        for (let i: number = 1; i < strandCount; ++i) {
            ...
            direction.x = ((CubismMath.cos(radian) * (direction.x as any)) - ((direction.y as any) * CubismMath.sin(radian)));
            direction.y = ((CubismMath.sin(radian) * direction.x) + ((direction.y as any) * CubismMath.cos(radian)));
            ...
            if (CubismMath.abs(strand[i].position.x as any) < thresholdValue) {
